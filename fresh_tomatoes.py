...
<head>
    <meta charset="utf-8">
    <title>Leonardo Nunes Carvalho</title>
...
movie_tile_content = '''
<div class="col-md-6 col-lg-4 movie-tile text-center" data-trailer-youtube-id="{trailer_youtube_id}" data-toggle="modal" data-target="#trailer"># noqa
    <img src="{poster_image_url}" width="220" height="342">
    <h2 class="movieTitle">{movie_title}</h2>
</div>
'''
...
def create_movie_tiles_content(movies):
    ...
    content = ''
    for movie in movies:
        youtube_id_match = re.search(r'(?<=v=)[^&#]+', movie.trailer_youtube)
        youtube_id_match = youtube_id_match or re.search(r'(?<=be/)[^&#]+', movie.trailer_youtube)  # noqa
